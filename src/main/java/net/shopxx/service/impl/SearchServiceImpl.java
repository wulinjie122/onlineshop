...
public class SearchServiceImpl implements SearchService {
        ...
        if (type == Article.class) {
            for (i = 0; i < this.articleDao.count(new Filter[0]); i += 20) {
                ...
                while (iterator.hasNext()) {
                    localObject = (Article) iterator.next();
                    fullTextEntityManager.index(localObject);
                }
            ...
            for (i = 0; i < this.productDao.count(new Filter[0]); i += 20) {
                ...
                iterator = list.iterator();
                while (iterator.hasNext()) {
                    localObject = (Product) iterator.next();
                    fullTextEntityManager.index(localObject);
                }
                fullTextEntityManager.flushToIndexes();
                fullTextEntityManager.clear();
    ...
    public Page<Product> search(String keyword, BigDecimal startPrice, BigDecimal endPrice, Product.OrderType orderType, Pageable pageable) {
        ...
        try {
            ...
            TermQuery termQuery = new TermQuery(new Term("sn", str));
            Query query1 = new QueryParser(Version.LUCENE_35, "keyword", new IKAnalyzer()).parse(str);
            ...
            booleanQuery1.add(termQuery, BooleanClause.Occur.SHOULD);
            ...
            booleanQuery1.add(localTermQuery2, BooleanClause.Occur.SHOULD);
            booleanQuery2.add(localTermQuery3, BooleanClause.Occur.MUST);
            booleanQuery2.add(localTermQuery4, BooleanClause.Occur.MUST);
            booleanQuery2.add(localTermQuery5, BooleanClause.Occur.MUST);
            ...
            if ((startPrice != null) && (startPrice.compareTo(new BigDecimal(0)) >= 0) && (endPrice != null) && (endPrice.compareTo(new BigDecimal(0)) >= 0)) {
                NumericRangeQuery numericrangequery = NumericRangeQuery.newDoubleRange("price",
                        Double.valueOf(startPrice.doubleValue()), Double.valueOf(endPrice.doubleValue()), true, true);
                booleanQuery2.add((Query) numericrangequery, BooleanClause.Occur.MUST);

            } else if ((startPrice != null) && (startPrice.compareTo(new BigDecimal(0)) >= 0)) {
                NumericRangeQuery numericRangeQuery = NumericRangeQuery.newDoubleRange("price", Double.valueOf(startPrice.doubleValue()), null, true, false);
                booleanQuery2.add((Query) numericRangeQuery, BooleanClause.Occur.MUST);

            } else if ((endPrice != null) && (endPrice.compareTo(new BigDecimal(0)) >= 0)) {
                NumericRangeQuery numericRangeQuery = NumericRangeQuery.newDoubleRange("price", null, Double.valueOf(endPrice.doubleValue()), false, true);
                booleanQuery2.add((Query) numericRangeQuery, BooleanClause.Occur.MUST);

            }
            FullTextEntityManager fullTextEntityManager = Search.getFullTextEntityManager(this.entityManager);
            FullTextQuery fullTextQuery = fullTextEntityManager.createFullTextQuery(booleanQuery2, new Class[]{Product.class});
